...
func add_point(x: float, y: float, z: float, u: float, v: float, u2: float = INF, v2: float = INF):
	...
	if using_legacy_uv2:
		# Prefab cells still supply explicit UV/UV2 data and expect the old blend path.
		uv = Vector2(u, v)
		uv2 = Vector2(u2, v2)
	else:
